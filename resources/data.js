  ...
  fs.readFile("deaths.csv", "utf8", function(error, stats) {
    ...
    regions.forEach(function(d) {
      ...
      if (boundaries.length > 0) {
        ...
        if (males.length > 0) region.properties.males = +males[0].Rate;
        if (females.length > 0) region.properties.females = +females[0].Rate;
        if (people.length > 0) region.properties.people = +people[0].Rate;
        newData.push(region);
      }
    });
    newData = {
